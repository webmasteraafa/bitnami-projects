<?php
     ...
     $num_of_recipes = 0;
     $allergens = getAllergensSelected();
     $connection;
     $currentPage = "SearchHandling";
     
     //Connect to Database
     makeConnectionToDb();
     //This will fill the results_array with all the search results
     handleSearch($_POST['keyword'], $_POST['course'], $allergens, $_POST['exclude']);

     //This session data will be accessed by search results page
     $_SESSION['db_result'] = $results_array;
     $_SESSION['total_rows'] = $num_of_recipes;
     $_SESSION['keyword'] = $_POST['keyword'];
     $_SESSION['course'] = $_POST['course'];
     $_SESSION['exclude'] = $_POST['exclude'];
     $_SESSION['allergens'] = $allergens;
     $_SESSION['from_page'] = 'search';
	 $_SESSION['query'] = $myquery;
     session_write_close();
      
	  //Location: SearchResultPage.php
     //Redirect to SearchResultsPage
     header( "Location: find-recipes.php");
     ob_flush();
     exit();     
     ...
     function handleSearch($selected_keyword, $selected_course, $selected_allergens, $selected_exclude){
        ...
        if($keyword != NULL){
		      ...
		      while($tok){
			     if($start == true){
					...
					$keyword_comm_string = "upper(SUBMITTER_COMMENTS) LIKE '%".strtoupper($tok)."%'";
					...
					$keyword_comm_string = $keyword_comm_string." OR upper(SUBMITTER_COMMENTS) LIKE '%".strtoupper(ltrim($tok))."%'";
        ...
        if($exclude != NULL){
		      ...
		      while($tok){
					...
					$exclude_comm_string = $exclude_comm_string." AND SUBMITTER_COMMENTS NOT LIKE '%".ltrim($tok)."%'";
        ...
        if($keyword != NULL && $course != NULL && $exclude == NULL){
           ...
           if($string == "")
               $query = "SELECT RECIPE_ID, NAME, SOURCE FROM TBL_RECIPE WHERE (".$keyword_name_string." OR ".$keyword_source_string." OR ".$keyword_comm_string.") ".$course_string." AND DELETED = 'N' AND APPROVED = 'Y'";
           else
               $query = "SELECT RECIPE_ID, NAME, SOURCE FROM TBL_RECIPE WHERE (".$keyword_name_string." OR ".$keyword_source_string." OR ".$keyword_comm_string." OR RECIPE_ID IN (".$string.")) ".$course_string." AND DELETED = 'N' AND APPROVED = 'Y'";
           $result = mysql_query($query) or trigger_error(mysql_error(), E_USER_ERROR);
        }
		$myquery = $query;
        if($keyword != NULL && $course != NULL && $exclude != NULL){
           ...
           $query = "SELECT RECIPE_ID FROM TBL_RECIPE WHERE (".$keyword_name_string." OR ".$keyword_source_string." OR ".$keyword_comm_string." ".$keyword_ingredients.") ".$course_string." AND DELETED = 'N' AND APPROVED = 'Y' ".$exclude_name_string." ".$exclude_source_string." ".$exclude_comm_string;
           $result = mysql_query($query) or trigger_error(mysql_error(), E_USER_ERROR);
           if($result == FALSE){
              mysql_free_result($result);
              return 0;
        ...
        if($keyword == NULL && $course != NULL && $exclude != NULL){
           $query = "SELECT RECIPE_ID FROM TBL_RECIPE WHERE DELETED = 'N' AND APPROVED = 'Y' ".$exclude_name_string." ".$exclude_source_string." ".$exclude_comm_string." ".$course_string."";
           $result = mysql_query($query) or trigger_error(mysql_error(), E_USER_ERROR);
           if($result == FALSE){
              mysql_free_result($result);
              return 0;
