...
//These statements control the image on the page
			...
			if ($pagetopic == 'medication-pharmacy') $smalldescrip = "Inactive versus active ingredients, generic or brand-name &#8212; there's a lot to consider before giving your food-allergic child medicine.";
			...
			if ($pagetopic == 'support_group') $smalldescrip = "Glean knowledge and emotional support from a support group of other parents facing similar challenges. Find a local food allergy support group or eosinophilic disease support group near you, or learn how to start a support group with help from Kids With Food Allergies. Support Group Leaders are offered a special forum to discuss outreach, events and advocacy.";
			...
			if ($pagetopic == 'rising-stars') $smalldescrip = "Real life stories of extraordinary children who happen to have food allergies. These inspirational stories help show that our children can do anything and are not hampered by their food allergies.<br><br>";
...
if ($pagetopic == 'allergy-friendly-new') {
 $title = "Allergy Information Resources";
 ...
 $description = "Nonprofit organization providing online support groups, recipes, parent education and news for families raising children with food allergies";
 }
if ($pagetopic == 'basics') {
...
if ($pagetopic == 'medication-pharmacy') {
 ...
 $description = "Inactive versus active ingredients, generic or brand-name; there's a lot to consider before giving your food-allergic child medicine. ";
 }
...
if ($pagetopic == 'supportnet') {
 $title = "Food Allergy Magazine | Support Net";
 $keywords = "food allergy, food allergies, food allergy magazine, Support Net, kids with food allergies, allergy free recipes, support, food allergy news, Rising Star, food allergy research, allergist, oral food challenges, school planning, restricted diet, epinephrine, traveling with food allergies,  ";
 $description = "Biannual print publication that focuses on children and families with food allergies.  Each issue features a food allergic child, recent food allergy news and research and supportive information.";
 }
 if ($pagetopic == 'freeguides') {
 $title = "Free Food Allergy Guides";
 $keywords = "food allergy, food allergies, food allergy brochure, Support Net, kids with food allergies, allergy free recipes, support, food allergy news, allergy friendly foods, hidden names for food allergens, chef card, food allergy diagnosis  ";
 $description = "These free guides are available for free download from Kids With Food Allergies.  Information about food allergies, hidden names for food allergens, diagnosis and testing and much more! ";
 }
 ...
 $keywords = "food allergy, food allergies, kids with food allergies, real life stories, peanut allergy, multiple food allergies, eosinophilic disorder, children with allergies";
 $description = "Read the real life stories of extraordinary children who happen to have food allergies.";
 }
 if ($pagetopic == 'travel') {
...
<div class="ltgreyboxmainR">
	...
	if ($pagetopic == 'supportnet') {
		include("resourcetopic-supportnet.php");
	}else if ($pagetopic == 'freeguides') {
		include("resourcetopic-freeguides.php");
	}else {
	echo "<div align='center' style='padding:8px 6px 8px 6px;' class='right'>
<script type='text/javascript' 
src='http://w.sharethis.com/button/sharethis.js#publisher=d1cb1749-92cb-45bf-b8fe-7146fdb014fb&amp;type=website&amp;popup=true&amp;style=rotate'></script></span></div>";
	echo "<img src=\"images/rec/".$theimage."\" alt=\"".$mytopic."\"  height=\"50\" class=\"left\"/><h2>";
	...
	$show_additional = '1';
		...
		while( $row = mysql_fetch_array( $result ) )
		{
			...
			if (($show_additional == 1) && ($show_premium != '1')) echo "<br />
<h2 style=\"text-align:left; margin-left:0px; font-size:14px; font-weight:bold; color: #D856A0; font-family: Arial, Helvetica, sans-serif; line-height:1.1\">Free Featured Resources:</h2>
";
		
			if ($time_passed < 60){
			echo "<h4><a border='0' href='resourcespre.php?id=".$row['id']."&title=".$urltitle."'>". stripslashes($row['title'])."</a>&nbsp;<b><font color='#CC3300'>NEW!</font></b></h4><div class='style24' style='padding-left:10px;'>".stripslashes($row['intro'])."</div><br />";
			} else {
			echo "<h4><a border='0' href='resourcespre.php?id=".$row['id']."&title=".$urltitle."'>".stripslashes($row['title'])."</a></h4><div class='style24' style='padding-left:10px;'>".stripslashes($row['intro'])."</div><br />";
			}
			$show_additional = $show_additional + 1;
			}else {
			if ($show_premium == '1' ) echo "<br /><h2 style=\"text-align:left; margin-left:0px; font-size:14px; font-weight:bold; color: #D856A0; font-family: Arial, Helvetica, sans-serif; line-height:1.1\">Premium Resources for Family Membership Subscribers:</h2>
";
			
			if ($time_passed < 60){
			echo "<h4 style='color:#3F679B;'>". stripslashes($row['title'])."&nbsp;<img src=\"http://kidswithfoodallergies.org/infopop/eve/star.gif\" alt=\"New\" />&nbsp;<b><font color='#CC3300'>NEW!</font></b></h4><div class='style24' style='padding-left:10px;'>".stripslashes($row['intro'])."</div><br />";
			} else {
			echo "<h4 style='color:#3F679B;'>".stripslashes($row['title'])."&nbsp;<img src=\"http://kidswithfoodallergies.org/infopop/eve/star.gif\" alt=\"New\" /></h4><div class='style24' style='padding-left:10px;'>".stripslashes($row['intro'])."</div><br />";
			}
...
<br />
<div align="center">
            <div style="border-width: 1px; border-style: solid; border-color: #3F679B; background-color: #D5E6F6; padding:2px; width:480px;text-align:center;">If you have   additional questions, you are welcome to become a member and connect with our volunteers and   other members on our <a href="http://community.kidswithfoodallergies.org" target="_blank">online support forums</a> who can be of additional help. </div></div>
			<!-- #EndEditable --></div>
</div><!--endsmaincolumn-->
</td></tr></table>
</div><!--endsmainbox-->
...
<!-- #BeginEditable "date" -->
      <p align="center" class="style24">Page last updated 01/30/2011</p>
      <!-- #EndEditable --></div>
